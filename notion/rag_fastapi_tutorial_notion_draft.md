...
이 튜토리얼은 예제 몇 개를 훑고 끝나는 문서가 아니다. `project_template/` 안에 실제로 실행되는 무장애 관광 챗봇을 만들면서 FastAPI, RAG, TourAPI, cache/fallback, 카드 근거, Web UI 검증까지 차례대로 익히는 초급자용 교재다.

끝까지 따라가면 `/tourism-ui/` 화면에서 “서울 강남구 휠체어 관광지 추천” 같은 질문을 넣을 수 있다. FastAPI `/tourism/chat`은 지역과 조건을 해석하고, 근거가 붙은 관광 카드 5장을 돌려준다. 설명은 문서에만 두고, 실제 화면과 API 응답은 사용자용 관광 상담 앱처럼 유지한다.

## 시작하기 전에

먼저 세 가지 원칙을 정해 둔다.

1. 사용자가 실제로 쓰는 완성형 앱을 만든다. 튜토리얼이라는 이유로 파일 구조나 화면을 줄이지 않는다.
2. 실행 산출물은 직접 만든다. `.env`, Chroma 런타임 DB, SQLite cache DB, 로그, `__pycache__`는 복사하지 않는다.
3. 외부 API가 없어도 학습이 끊기지 않게 한다. 프로젝트에 들어 있는 관광 markdown seed와 live markdown cache를 학습용 안전망으로 쓰고, live TourAPI는 준비된 뒤 연결한다.
...
각 장은 같은 흐름으로 읽으면 된다. 먼저 “이번 장에서 만들 것”을 보고, 파일을 만들거나 확인한 뒤 pytest 또는 노트북으로 검증한다. 막히면 같은 장의 “자주 나는 오류와 해결”부터 확인한다.

## 1장. 환경 준비

여기서는 튜토리얼과 앱을 같은 Python 환경에서 실행할 준비를 한다.
...
이 장을 마치면 내 컴퓨터에서 FastAPI 앱을 띄울 준비가 끝난다.

## 2장. FastAPI 앱 만들기

`project_template/app/main.py`에 FastAPI 앱을 만들고 `/health`를 붙인다. 아직 RAG나 관광 추천은 넣지 않는다. 먼저 서버가 살아 있는지 확인할 수 있는 작은 엔드포인트부터 만든다.
...
RAG에 넣을 원천 문서를 읽는 흐름을 만든다. 여기서 “파일을 읽는다”와 “답변에 쓸 근거를 준비한다”가 어떻게 다른지 짚고 넘어간다.
...
긴 문서를 검색 가능한 조각으로 나누고, 임베딩 서비스에 넣을 입력을 만든다. 이 장에서는 실제 LLM 답변보다 검색할 수 있는 형태로 바꾸는 과정이 더 중요하다.
...
저장된 청크에서 질문과 관련 있는 근거를 찾아온다. 이 단계부터 RAG의 검색이 눈에 보이기 시작한다.
...
검색 근거를 프롬프트에 넣고 답변을 만드는 흐름을 연결한다. 여기서 LLM이 그냥 아는 척 답하는 경우와, 검색 근거를 보고 답하는 경우를 구분한다.
...
사용자의 질문을 HTTP 요청으로 받고 답변 JSON을 반환한다. 이 장부터 프론트엔드가 기대할 API 계약이 생긴다.
...
관광 후보를 가져오는 서비스 계층을 만든다. API 키가 있으면 한국관광공사 TourAPI를 호출하고, 키가 없거나 네트워크가 불안하면 다음 장에서 다루는 저장 자료 흐름으로 넘어가게 만든다.
...
- API 키는 `.env`에만 둔다.
- live 조회는 마지막 보강이다. 초급자가 첫 실행을 성공시키기 위한 필수 조건은 아니다.
...
외부 API 없이도 관광 추천 흐름을 끝까지 볼 수 있게 만든다. 이 장에서는 관광 markdown seed와 live markdown cache를 학습 재료로 사용한다. 이것은 나중에 덧붙일 데이터가 아니라, 초급자가 10-14장을 따라가기 위해 필요한 기본 재료다.
...
사용자가 입력한 “서울”, “강남구”, “부산 해운대구” 같은 말을 앱이 지역 코드로 해석하게 만든다.
...
사용자가 “더 보기”, “조건 완화하기”, “같은 시도까지 넓히기”처럼 짧게 말해도 이전 질문의 맥락을 이어받게 만든다.
...
답변 문자열만 반환하지 않는다. 사용자가 비교할 수 있도록 관광 카드와 출처를 함께 돌려준다.
...
앱이 제대로 답했는지 반복해서 확인할 수 있도록 평가 스크립트와 이벤트 로그를 둔다. 운영자를 위한 장이지만, 초급자는 테스트 데이터로 같은 질문을 다시 확인하는 법을 배운다.
...
최종 사용자가 보는 `/tourism-ui/` 화면을 확인한다. 이 화면은 튜토리얼 설명 화면이 아니라 실제 무장애 관광 상담 화면이어야 한다.
...
로컬 서버, debug tunnel, release tunnel, 테스트 명령을 같은 규칙으로 실행한다.
...
지금 읽고 있는 원고를 Notion 페이지에 순서형 교재로 옮긴다. Notion에서는 변경 내역을 위에 쌓지 않는다. 초급자가 위에서 아래로 읽으면서 1장부터 16장까지 따라갈 수 있어야 한다.
...
- 실제 UI와 API가 사용자용 화면과 응답이어야 한다는 원칙
...
노트북은 설명 셀과 코드 셀을 번갈아 두는 강의형 자료다. 각 노트북은 최소 11셀 이상, 마크다운 5셀 이상, 코드 6셀 이상을 가지며, 실제 템플릿은 18-20셀 규모다.
